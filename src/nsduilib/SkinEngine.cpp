...
int CSkinEngine::SetIconRes(TCHAR* iconname)
{
    ...
    this->m_pIconInfo = CRenderEngine::LoadImage(iconname, NULL, 0);
    if (this->m_pIconInfo == NULL) {
        ERROR_INFO("get (%s) error(%d)", T2A(iconname), GetLastError());
        return -1;
    }
    ::PostMessage(this->m_hWnd, WM_SETICON, (WPARAM) FALSE, (LPARAM) CreateIcon(this->m_pIconInfo->hBitmap));
    return 0;
}
